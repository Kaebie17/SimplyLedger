...
contentArea.style.height = screen.height - header.getBoundingClientRect().height - header.nextElementSibling.getBoundingClientRect().height - footer.getBoundingClientRect().height + "px" ;
...
async function createCalendar(d){
    ...
    newentryBtn.addEventListener("touchend", handleAddNew = function(e){
        ...
        yesorno.addEventListener("touchend",(e)=>{
            ...
            if (!yesorno.classList.contains("yes")) {
                ...
                constituents[name] = bool ? {name: name, value: val*1, quantity: qty*1, subCategory : subCat, storeName:store} : {name: name, value: val*1, quantity: qty*1, subCategory : subCat};
...
function setValue(request,expenseTD){
    ...
    console.log(expenseTD,request.value);
    return expenseTD
}
...
function showPlotFunction(e,period,baseDt=new Date()){
    ...
    if (plotsArea.classList.contains("hidden")) {
            ...
            let start = period === "weekly"? (new Date(baseDt - 7*7*24*60*60*1000))  : period === "monthly"? (new Date(baseDt - 7*30*24*60*60*1000))  : new Date(baseDt - 7*24*60*60*1000);
            let end = baseDt||new Date();
        ...
        storeDBData((arr)=>arr.filter(({timestamp,...o})=>new Date(timestamp)>=start && new Date(timestamp)<=end),(arr,evt)=>groupData(arr,period||"daily",transformDataFromDB,evt),e)
        }
    }
...
function storeDBData(filter,callback,evt){
    ...
    promise.onsuccess = (res) => {
        let fullDBExtract = !filter ? res.target.result : filter(res.target.result) ;
        if(!fullDBExtract.length) { return;}
        if(evt.target.localName === "canvas") evt.target.parentElement.replaceChildren();
        callback(fullDBExtract,evt);
        createCanvas ()
    }
...
function transformDataFromDB(dbData,bool,e){
    ...
    console.log(subcategoryPlotData);
    ...
    if (period==="daily"){
        ...
        for (let i=0; i<len; i++){
            ...
            if (period==="weekly") {
                ...
                periodStart = p+1 > 52 ? `1-${y+1}` : `${p+1}-${y}`;
            }
            else {
                let k;
...
function swipeChart(z){
        ...
        z.target.addEventListener("touchend",(e)=>{
            ...
            if (Math.abs(swipeDistanceX)>150 && swipeDistanceX < 0) {
                if (freq === "daily" ) start = new Date(new Date(e.target.dataset.date).getTime()+7*24*60*60*1000);
                else if (freq === "weekly" ) start = new Date(getDateFromWeek(e.target.dataset.date).getTime()+7*24*60*60*1000);
                else{
                    ...
                    start = new Date(new Date(`${mt}-${dates}-${yr}`).getTime() + dates*24*60*60*1000);
                }
                incomeByDate = {};
                categoryPlotData = {};
                subcategoryPlotData = {};
                ...
                subcategoryDatePlotData = {};
                valuesPlotData = {};
                showPlotFunction(e,timelines[changePlotTimelines.counter],start);
            }
            if (Math.abs(swipeDistanceX)>150 && swipeDistanceX > 0) {
                // debugger
                if (freq === "daily" ) start = new Date(new Date(e.target.dataset.date).getTime()-7*24*60*60*1000);
                else if (freq === "weekly" ) start = new Date(getDateFromWeek(e.target.dataset.date).getTime()-7*7*24*60*60*1000);
                else{
                    let [mt,yr] = e.target.dataset.date.split("-");
                    mt = mt*1;
                    yr = (20+yr)*1;
        ...
        for(let uDate of uniqueDates){  
            ...
            dbArr.filter(({timestamp,...o}) => timestamp===uDate).forEach(({timestamp,value,category,detail},i) => {
                ...
                contentBoxClone.addEventListener("touchend",(e)=>{
                    ...
                    target.localName === "fieldset" ?  (createSwipeFunction({elm:target,nextFn,prevFn},"legend","svg"),createSwipeFunction({elm:target,targetEL:svgEL,nextFn:nextmarkerFn,prevFn:prevmarkerFn,idFn: elem => elem.getAttribute("fill")==="black"})) : "" ;
                })
...
function createSwipeFunction({elm,targetEL=elm,nextFn,prevFn,idFn=el => !el.classList.contains("hidden")},...filters){
...
function getWeek(date=new Date(),reverse){
    let first = new Date(`01/01/${new Date(date).getFullYear()}`);
...
function getDateFromWeek(week,num){
    ...
    yr = (20+yr)*1 ;
    mt = mt*1;
    let firstDate = new Date(`01/01/${yr}`);
    let gap = 7 - firstDate.getDay() - 1;
    let nextWeekStart = new Date(`01/${7-gap}/${yr}`)
    let secondWeek = getWeek(nextWeekStart).split("-")[0]*1;
    if (mt === secondWeek) {return new Date(nextWeekStart.getTime()+(num??6)*24*60*60*1000)}
        ...
        return new Date(nextWeekStart.getTime()+(num??6)*24*60*60*1000)
    }
}
        ...
        reader.onload = () =>{
            ...
            for (let line of dataArray){
                ...
                if (dbInputObj?.[key]){
                    ...
                    dbInputObj[key] = {timestamp,category,value:total*1,detail}
                }
            }
    ...
    request.addEventListener("upgradeneeded", (event) => {
        ...
        if( !db.objectStoreNames.contains(dbname)){
            for (let store of stores){
                ...
                indexKeys = Object.keys(indexObject);
